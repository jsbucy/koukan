...
Koukan is intended to be compatible with horizontal scaling of both
the router and gateway via a cluster scheduler such as Kubernetes
(k8s). In particular the router does stores all durable data in a
database and does not assume a durable/strongly consistent posix
filesystem. However the router does stage some inflight data through
the filesystem but this does not need to be durable; EmptyDir is
fine. The smtp gateway should not require a writable filesystem at
all.
