...
class SMTP:
    ...
    def bdat(self, msg, last):
        cmd = b'bdat %d' % len(msg)
        if last:
            cmd += b' last'
        cmd += bCRLF
        self.send(cmd)
    ...
    def data_chunk(self, msg, last : bool):
        return self.data(msg, last)

    def data(self, msg, last=True):
        ...
        If last is False, does not send the final .CRLF so large
        messages can be sent in multiple chunks while being read from
        a file, etc.
        """

        do_bdat = self.enable_bdat and self.has_extn('chunking')
        # buffer crlf that spans chunk boundaries: if a chunk prior to
        # the final one possibly ends with a line ending, save that
        # for the next call so _fix_eols()/_quote_periods() will work.
        if self._end:
            msg = self._end + msg
            self._end = None
        if not last and (not do_bdat or isinstance(msg, str)):
            for e in LINE_ENDINGS if isinstance(msg, str) else bLINE_ENDINGS:
                if msg.endswith(e):
                    self._end = e
                    msg = msg[:-len(e)]
